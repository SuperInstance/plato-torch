...
Supports the preset system: pass preset="reinforce", preset="supervised", etc.
to configure the room for a specific training paradigm. Presets override
feed/train/predict/export while keeping the core observe→train→simulate→instinct
pipeline intact.
"""
...
class TorchRoom(RoomBase):
    ...
    Preset support:
        ...
        room.feed(data)          # preset-specific data ingestion
        room.train()             # preset-specific training
        room.predict("state")    # preset-specific inference
        room.export(format="json")  # preset-specific export
    ...
    def export(self, format: str = "json", path: str = None, **kwargs) -> str:
        ...
        export_path = path or str(self.ensign_dir / f"export.{format}")

        if format == "json":
            with open(export_path, "w") as f:
                json.dump(model, f, indent=2)
        else:
            # For GGUF, SafeTensors, ONNX — write JSON metadata stub
            # Real export would require PyTorch + format-specific libs
            meta = {
                "room_id": self.room_id,
                "preset": self.preset_name,
                "format_requested": format,
                "model_stats": model.get("stats", {}),
                "note": f"Full {format} export requires PyTorch + format-specific dependencies"
            }
            meta_path = export_path + ".meta.json"
    ...
    def train(self, epochs: int = 10, batch_size: int = 32, **kwargs):
    ...
    def __repr__(self):
        ...
        preset_str = f", preset={self.preset_name}" if self.preset_name else ""
        return f"TorchRoom({self.room_id}, {self.use_case}, {trained}, tiles={self._episodes_seen}{preset_str})"
